...
export class UserRateLimiterInterceptor implements NestInterceptor {
  ...
  private async checkUserRateLimit(userInfo: UserClaims, response: Response): Promise<void> {
    ...
    const windowStart = this.calculateUserWindowStart(currentTime, createdAt, windowSeconds);
    ...
    response.setHeader('X-RateLimit-Remaining', Math.max(0, maxRequests - newCount).toString());
    response.setHeader('X-RateLimit-Reset', Math.floor(windowEnd.getTime() / 1000).toString());
  }

  private async checkIPRateLimit(request: Request, response: Response): Promise<void> {
    ...
    const windowStart = Math.floor(currentTime.getTime() / 1000 / windowSeconds) * windowSeconds;
  ...
  private calculateUserWindowStart(currentTime: Date, createdAt: Date, windowSeconds: number): Date {
    const timeSinceCreation = currentTime.getTime() - createdAt.getTime();
    const windowsSinceCreation = Math.floor(timeSinceCreation / (windowSeconds * 1000));
    return new Date(createdAt.getTime() + windowsSinceCreation * windowSeconds * 1000);
  }
}
