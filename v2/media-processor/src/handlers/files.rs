...
pub async fn upload_file(
    ...
    while let Some(item) = multipart.next().await {
        ...
        let content_disposition = field
            .content_disposition()
            .ok_or_else(|| error::ErrorBadRequest("Content-Disposition header missing"))?
            .clone();
        ...
        match field_name {
            "file" => {
                ...
                content_type = field
                    .content_type()
                    .map_or("application/octet-stream".to_string(), |ct| ct.to_string());
    ...
    if let Some(existing_file) = file_repo
        .get_ref()
        .get_file_by_md5_hash(&md5_hash)
        .await
        .map_err(|e| error::ErrorInternalServerError(format!("Database error: {}", e)))?
    {
    ...
    let (file_url, thumbnail_url) = s3_repo
        .get_ref()
    ...
    if let Err(e) = file_repo.get_ref().create_file(&file).await {
        let _ = s3_repo
            .get_ref()
            .delete_file(&file_id, &Some(file_url))
            .await;
...
pub async fn delete_file(
    ...
    s3_repo
        .get_ref()
        .delete_file(&file_id, &file.stored_filename)
        .await
        .map_err(|e| error::ErrorInternalServerError(format!("S3 error: {}", e)))?;
...
pub async fn get_file_content(
    ...
    if download {
        ...
        let data = s3_repo
            .get_ref()
            .get_file(&file_id, &file.stored_filename)
            .await
            .map_err(|e| error::ErrorInternalServerError(format!("S3 error: {}", e)))?;
...
pub async fn get_thumbnail(
    ...
    let size = query
        .get("size")
        .unwrap_or(&"small".to_string())
        .to_string();

    // Try to get existing thumbnail first
    match s3_repo
        .get_ref()
        .get_thumbnail(&file_id, &file.thumbnail_filename, &size)
        .await
    {
        Ok(data) => {
            let mut response = HttpResponse::Ok();
        ...
        Err(_) => {
            ...
            if file.mime_type.starts_with("image/")
                && thumbnail_gen.is_image_format_supported(&file.mime_type)
            {
                // Get original file data
                let original_data = s3_repo
                    .get_ref()
                    .get_file(&file_id, &file.stored_filename)
                    .await
                    .map_err(|e| error::ErrorInternalServerError(format!("S3 error: {}", e)))?;
                ...
                // Generate thumbnail
                let thumbnail_data = thumbnail_gen
                    .generate_thumbnail(&original_data, &size)
                    .map_err(|e| {
...
pub async fn get_file_stats(file_repo: web::Data<FileRepository>) -> Result<HttpResponse, Error> {
    let stats = file_repo
        .get_ref()
        .get_file_stats()
        .await
...
pub async fn purge_files(
    ...
    let except_board_ids = if let Some(board_ids) = request.except_board_ids.as_ref() {
        let uuid_board_ids: Result<Vec<Uuid>, _> =
            board_ids.iter().map(|id| Uuid::parse_str(id)).collect();
    ...
    let (estimated_files, estimated_size) = file_repo
        .get_ref()
        .estimate_purge(
